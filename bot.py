...
# Конфигурация
MAIN_BOT_TOKEN = os.getenv('MAIN_BOT_TOKEN')  # Токен основного бота из переменных окружения
...
if not MAIN_BOT_TOKEN:
    raise ValueError("MAIN_BOT_TOKEN не установлен в переменных окружения")
...
class CryptoBotAPI:
    ...
    async def create_invoice(self, amount, currency_type="USD", description=""):
...
async def run_shop_bot(bot_token, bot_name, admin_id, crypto_api):
    ...
    async def shop_select_product(callback_query: types.CallbackQuery):
        ...
        description = f"Покупка товара {product['name']} в магазине {bot_name}"
    ...
    async def shop_check_payment(callback_query: types.CallbackQuery):
        _, invoice_id, product_id = callback_query.data.split("_")
    ...
    async def shop_about(callback_query: types.CallbackQuery):
        ...
        channel_url = settings['channel_url'] if settings and settings['channel_url'] else "Не указан"
        
        keyboard = InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Назад", callback_data="back_to_menu"))
        
        await callback_query.message.answer(
            f"ℹ️ О нас\n\n"
            f"🏪 Название: {bot_info['bot_name']}\n"
            f"📅 Дата создания: {bot_info['created_at'].strftime('%d.%m.%Y')}\n"
            f"📢 Канал: {channel_url}",
            reply_markup=keyboard
        )
    ...
    async def shop_admin(callback_query: types.CallbackQuery):
        ...
        balance_text = ""
        if balance:
            usdt_balance = next((item for item in balance if item['currency_code'] == 'USDT'), None)
            if usdt_balance:
                balance_text = f"\n💰 Баланс: {usdt_balance['available']} USDT"
...
async def main_start(message: types.Message):
    ...
    keyboard.add(InlineKeyboardButton("➕ Создать нового бота", callback_data="create_bot"))
...
async def process_crypto_api(message: types.Message, state: FSMContext):
    ...
    try:
        test_crypto = CryptoBotAPI(message.text)
...
async def main():
    ...
    for bot in bots:
        ...
        print(f"Запущен бот: {bot['bot_name']}")
